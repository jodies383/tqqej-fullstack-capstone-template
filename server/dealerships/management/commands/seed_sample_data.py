...
class Command(BaseCommand):
    def handle(self,*args,**kwargs):
        ...
        with connection.cursor() as cursor:
            cursor.execute("DELETE FROM sqlite_sequence WHERE name IN ('dealerships_review', 'dealerships_dealer', 'dealerships_carmake', 'dealerships_carmodel')")
        rows=[
        ...
        cities = [("Austin", "Texas"), ("Phoenix", "Arizona"), ("Seattle", "Washington"), ("Miami", "Florida"), ("Portland", "Oregon"), ("Boston", "Massachusetts")]
        for number in range(5, 51):
            city, state = cities[(number - 5) % len(cities)]
            rows.append((f"Community Auto {number}", city, state, f"{number} Market Street, {city}, {state}", f"555-010-{number:02d}"))
        ...
        for name, models in {"Toyota":["Camry","Corolla","RAV4"],"Honda":["Civic","Accord","CR-V"],"Ford":["Mustang","F-150","Explorer"],"Nissan":["Altima","Rogue","Sentra"],"Chevrolet":["Malibu","Equinox","Silverado"]}.items():
